...
// ==========================================================
// 🚨 NEW: USER LOGIN CHECK
// Assume 'user_id' is set in the session after a successful login
...
/* 错误提示框样式 (Adjusted for general use) */
.error-content {
...
<header>
    ...
    <nav>
        ...
        <a href="my_order.php">Track Orders</a> <a href="about.html">About</a>
...
// --- NEW: Login Status Variable ---
const IS_LOGGED_IN = <?= $is_logged_in ? 'true' : 'false' ?>;
let errorCallback = null; // Store a function to run after the error modal closes
...
// --- 错误提示 UI (替换 alert) ---
// Updated to accept an optional callback function
function showErrorModal(title, message, callback = null) {
    document.getElementById("errorModal").querySelector('h3').innerText = title;
    document.getElementById("errorMessage").innerText = message;
    errorCallback = callback; // Store the callback
    document.getElementById("errorModal").style.display = "flex";
}
function closeErrorModal() {
    document.getElementById("errorModal").style.display = "none";
    if (errorCallback) {
        errorCallback(); // Execute the callback if it exists
        errorCallback = null; // Clear the callback
    }
}
...
function prepareOrder(gameId) {
    // 1. **NEW: CHECK LOGIN STATUS FIRST**
    if (!IS_LOGGED_IN) {
        // Close the current modal before showing the error/redirect
        closeModal(gameId); 
        ...
        showErrorModal(
            ...
            function() {
                // Redirect to signin.php after user clicks OK
    ...
    if (!cart[gameId] || Object.keys(cart[gameId]).length === 0) {
        showErrorModal("Selection Required", "Please select at least one item before proceeding to payment.");
        return false; // 阻止表单提交
    }
    
    // 3. 将选中的物品数据转换为 JSON 字符串
